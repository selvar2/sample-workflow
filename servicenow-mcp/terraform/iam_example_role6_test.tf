#############################################################################
# Terraform Configuration for IAM Role: example_role6
# 
...
#
# Role Details:
#   - Role Name: example_role6_mcp_test
#   - Path: /
...
#############################################################################
# IAM Policy: example_policy6_mcp
# 
# Policy document identical to example_policy3_mcp_test.
#############################################################################
resource "aws_iam_policy" "example_policy6_mcp" {
  name        = "example_policy6_mcp_test"
  path        = "/"
...
#############################################################################
resource "aws_iam_role_policy_attachment" "example_role6_mcp_attachment" {
  role       = aws_iam_role.example_role6_mcp.name
  policy_arn = aws_iam_policy.example_policy6_mcp.arn
}
...
#############################################################################
output "role6_mcp_role_arn" {
  description = "ARN of the IAM role created via MCP"
  value       = aws_iam_role.example_role6_mcp.arn
}

output "role6_mcp_role_name" {
  description = "Name of the IAM role created via MCP"
  value       = aws_iam_role.example_role6_mcp.name
}

output "role6_mcp_role_id" {
  description = "Unique ID of the IAM role created via MCP"
  value       = aws_iam_role.example_role6_mcp.unique_id
}

output "role6_mcp_policy_arn" {
  description = "ARN of the IAM policy created via MCP"
  value       = aws_iam_policy.example_policy6_mcp.arn
}
