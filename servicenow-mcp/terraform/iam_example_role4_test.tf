#############################################################################
# Terraform Configuration for IAM Role: example_role4
# 
...
#############################################################################
# IAM Policy: example_policy4_mcp
# 
# Policy document identical to example_policy3_mcp_test.
#############################################################################
resource "aws_iam_policy" "example_policy4_mcp" {
...
#############################################################################
resource "aws_iam_role_policy_attachment" "example_role4_mcp_attachment" {
  role       = aws_iam_role.example_role4_mcp.name
  policy_arn = aws_iam_policy.example_policy4_mcp.arn
}
...
#############################################################################
output "role4_mcp_role_arn" {
  description = "ARN of the IAM role created via MCP"
  value       = aws_iam_role.example_role4_mcp.arn
}

output "role4_mcp_role_name" {
  description = "Name of the IAM role created via MCP"
  value       = aws_iam_role.example_role4_mcp.name
}

output "role4_mcp_role_id" {
  description = "Unique ID of the IAM role created via MCP"
  value       = aws_iam_role.example_role4_mcp.unique_id
}

output "role4_mcp_policy_arn" {
  description = "ARN of the IAM policy created via MCP"
  value       = aws_iam_policy.example_policy4_mcp.arn
}
