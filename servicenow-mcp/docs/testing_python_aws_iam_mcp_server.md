# Testing Python and AWS IAM MCP Server
...
Using Python (boto3) and the AWS IAM MCP server, inspect the existing IAM role `example_role6_mcp_test`, retrieve all attached permissions and policies, and add the `s3:ListBucket` permission to the role.

---

## Python Script

**File:** `scripts/add_s3_listbucket_permission.py`
...
```bash
cd /workspaces/sample-workflow/servicenow-mcp && python3 scripts/add_s3_listbucket_permission.py
```
...
```
============================================================
AWS IAM Role Permission Update
Role: example_role6_mcp_test
============================================================
✅ Role Found: example_role6_mcp_test
   ARN: arn:aws:iam::175853813947:role/example_role6_mcp_test
   Role ID: AROASR4NUIC5W7DGT3ICG
   Created: 2025-12-31 11:32:17+00:00

📎 Attached Managed Policies: 1
   - example_policy6_mcp_test (arn:aws:iam::175853813947:policy/example_policy6_mcp_test)

📝 Inline Policies: 0

🔧 Working with managed policy: example_policy6_mcp_test

📄 Current Policy Document (Version: v1):
{
  "Statement": [
    {
      "Action": [
...
🔍 Validation - New Policy Document (Version: v2):
{
  "Statement": [
    {
      "Action": [
...
CONFIRMATION
============================================================
✅ Role ID unchanged: True
   Original: AROASR4NUIC5W7DGT3ICG
   Current:  AROASR4NUIC5W7DGT3ICG
✅ Policy type: MANAGED (not inline)
✅ Role was modified in place, NOT recreated
```
...
|------|---------|
| **Role** | `example_role6_mcp_test` |
| **Role ARN** | `arn:aws:iam::175853813947:role/example_role6_mcp_test` |
| **Role ID** | `AROASR4NUIC5W7DGT3ICG` (unchanged) |
| **Policy Updated** | `example_policy6_mcp_test` |
| **Policy Type** | **Managed** (not inline) |
...
- ✅ `s3:ListBucket` is now present
- ✅ Existing permissions (`s3:GetObject`, `s3:PutObject`) preserved
- ✅ Role was **modified in place**, not recreated
- ✅ Role ID unchanged
- ✅ Trust policy preserved
...
`scripts/add_s3_listbucket_permission.py`
